...
function createCalloutAnnouncer(audioQueue) {
    ...
    switch (feature.geometry.type) {
      case 'LineString':  // e.g. roads
        feature.point = turf.nearestPointOnLine(
          feature, audioQueue.locationProvider.turfPoint(), { units: "meters"}
        );
        break;
    ...
    feature.announce = (options) => {
      return feature.getAudioLabel().then(label => {
        if (label) {
          ...
          playSoundAndSpeech(feature.soundEffect, label, feature.point, options.includeDistance);
          return true;
        } else {
          return false;
        }
  ...
  const announcer = {
    ...
    calloutNearestRoad: (latitude, longitude) => {
      const radiusMeters = audioQueue.locationProvider.radiusMeters;
      announcer.nearbyRoads(latitude, longitude, radiusMeters)
      .then(roads => {
        if (roads.length > 0) {
          playSoundAndSpeech(
            'sense_mobility', `Nearest road: ${roads[0].properties.name}`, roads[0].point, true
          );
        }
      });
    },
